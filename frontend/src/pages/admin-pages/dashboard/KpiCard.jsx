...
export const KpiCard = () => {
  ...
  const fetchDashboardMetrics = async () => {
    try {
      ...
      const kpis = [
        { icon: <Building2 />, label: "Total Branches", value: data.total_branches },
        { icon: <Users />, label: "Employees", value: data.total_employees },
        { icon: <DollarSign />, label: "Total Sales", value: `$${data.total_sales}` },
        { icon: <Flame />, label: "Active Promotions", value: data.active_promotions },
      ];
      setMetrics(kpis);
    } catch (error) {
      ...
      {metrics.map((m, i) => (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-[#704f34] font-medium tracking-wide">{m.label}</p>
              <p className="text-2xl font-extrabold text-[#3f2c1d]">{m.value}</p>
            </div>
          </div>
        </div>
      ))}
