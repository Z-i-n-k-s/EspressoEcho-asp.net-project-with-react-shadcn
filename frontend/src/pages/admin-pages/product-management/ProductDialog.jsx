...
export default function ProductDialog({
  ...
  const saveItem = () => {
      if (!selectedBranch || !currentItem) return;
  
      // Basic validation & guards
      const qty = Number(currentItem.quantity) || 0;
      const rl = Number(currentItem.reorderLevel) || 0;
      const bp = Number(currentItem.basePrice) || 0;
      ...
      const sanitized = {
        ...
        item: String(currentItem.item || "").trim(),
        category: String(currentItem.category || "").trim(),
      };
      ...
      // Ensure category exists for this branch; if not, add it.
      const catExists = branchCategories.some(
        (c) => c.name === sanitized.category
      );
      if (!catExists && sanitized.category) {
        upsertBranchCategories((prev) => [
          ...prev,
          { id: Date.now(), name: sanitized.category },
        ]);
      }
  
      if (isAddMode) {
        setCurrentInventory((prev) => [...prev, sanitized]);
      } else {
        setCurrentInventory((prev) =>
          prev.map((inv) => (inv.id === sanitized.id ? sanitized : inv))
        );
      }
