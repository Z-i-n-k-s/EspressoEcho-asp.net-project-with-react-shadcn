...
-- ===================================================================
-- ORDER CANCELLATION - PROMOTION RESTORATION TRIGGER
-- ===================================================================

DELIMITER //

CREATE TRIGGER tr_order_cancellation_promo_restore
AFTER UPDATE ON orders
FOR EACH ROW
BEGIN
    DECLARE promo_id CHAR(36);
    DECLARE customer_promo_id CHAR(36);
    
    -- Check if order was cancelled and had a promo code
    IF NEW.order_status = 'cancelled' AND OLD.order_status != 'cancelled' AND NEW.promo_code_used IS NOT NULL THEN
        -- Get the promotion ID
        SELECT id INTO promo_id
        FROM promotions 
        WHERE code = NEW.promo_code_used;
        
        -- Find the customer_promotions record for this order
        SELECT id INTO customer_promo_id
        FROM customer_promotions
        WHERE order_id = NEW.id
        AND promo_id = promo_id
        AND status = 'used';
        
        -- If found, restore the promotion to the customer
        IF customer_promo_id IS NOT NULL THEN
            -- Update customer_promotions record to reassign the promotion
            UPDATE customer_promotions
            SET status = 'assigned',
                used_at = NULL,
                order_id = NULL,
                updated_at = CURRENT_TIMESTAMP
            WHERE id = customer_promo_id;
...
-- ===================================================================
-- DELIVERY MANAGEMENT TRIGGERS
-- ===================================================================

DELIMITER //

-- Auto-Complete Orders on Delivery
CREATE TRIGGER tr_auto_complete_delivered_orders
...
BEGIN
    ...
    IF NEW.status = 'delivered' AND OLD.status != 'delivered' THEN
        ...
        -- Update delivery assignment completed_at if not already set
